 ...
 * built for the two GLP-1 medications. These 5 products bill Monthly, with
 ...
 *
 * FIRST PASS PRICING (2026-08-27): set at roughly 1.5x landed cost (pharmacy
 * + dispense + shipping + doctor's fee, exactly as summed in the "Beema
 * Pricing" cost sheet), charm-rounded. TRT, hairloss, and ED costs come from
 * that sheet. NAD+ and sermorelin have no entry in that sheet at all - their
 * prices are telehealth market-rate estimates instead, cross-checked against
 * Bask catalog price ranges ($65-$240 for NAD+, $85-$264 for sermorelin
 * variants), not derived from a landed cost. Revisit both once real pharmacy
 * costs exist. Dose does not change price, matching the semaglutide/
 * tirzepatide convention.
 */
...
/** Enclomiphene, Partell Pharmacy landed cost (25mg, ceiling dose) $112.99 x1.5. No quarterly SKU in the cost sheet. */
export const TRT_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 169,
};

/** Oral minoxidil, GoGoMeds. Monthly cost $65.99 x1.5; quarterly cost $71.99 x1.5. */
export const HAIRLOSS_ORAL_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 99,
  quarterly: quarterlyPlan(109, 99),
};

/** Compounded topical spray, Pharmacy Hub. Monthly cost ~$85 x1.5; quarterly cost ~$147.99 x1.5. */
export const HAIRLOSS_TOPICAL_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 129,
  quarterly: quarterlyPlan(219, 129),
};

/** Single-ingredient sildenafil/tadalafil, Health Warehouse. Monthly cost ~$66.50 x1.5; quarterly cost ~$69.50 x1.5. */
export const ED_STANDARD_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 99,
  quarterly: quarterlyPlan(89, 99),
};

/** Tadalafil + sildenafil combo RDT, Pharmacy Hub. Monthly cost $85.39 x1.5; quarterly cost ~$107.79 x1.5. */
export const ED_COMBO_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 129,
  quarterly: quarterlyPlan(159, 129),
};

/** No cost-sheet entry - market-rate estimate, cross-checked against Bask's own $65-$240 variant range. */
export const NAD_PRICING: SimpleCompoundedPricing = {
  monthlyUsd: 149,
};

/** No cost-sheet entry - market-rate estimate, cross-checked against Bask's own $189 suggested retail / up to $264 variant range. */
