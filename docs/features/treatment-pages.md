...
Each medication Beema offers gets its own indexable, SEO-focused **money page** (`/tirzepatide`, `/semaglutide`, `/tadalafil`, `/sildenafil`, `/oral-finasteride`, `/ed-mints`) - this targets each drug's search terms without diluting them, and gives each page its own FAQPage/BreadcrumbList JSON-LD. Beema's patient-facing offering is compounded across most product lines - **oral finasteride, tadalafil, and sildenafil are the exceptions: real FDA-approved generics, not compounded** (corrected 2026-09-03 for tadalafil/sildenafil, per Matt - see the product table below). ED Mints remains compounded (a combination formulation not sold as a single commercial product by any manufacturer). Do not add branded-medication pages (Wegovy, Zepbound, Ozempic, Mounjaro, or any branded ED product) or describe those brands as Beema offerings. `/weight-loss` and `/ed` sit alongside the money pages as broader overview pages targeting head-term searches ("medical weight loss," "ED treatment"), and `/hair-loss` is a category hub playing the same head-term role for "hair loss treatment" - see "Money-page architecture" below for how these page types relate.
...
**`/ed` is an overview page, not a money page** (restructured 2026-09-03 - see "Money-page architecture" below). **`/hair-loss` is a category hub, not a money page** (see "Category hub pages" below) - it merged what used to be two separate pages, `/hair` (a hub) and `/hairloss` (a 6-SKU overview/FAQ page), into one on 2026-09-04, since neither had shipped to production and having both was redundant, SEO-diluting duplication. `HAIR_SECTIONS` in `SiteHeader.tsx` still lists only Oral Finasteride, with Oral Minoxidil and the men's Topical Spray commented out - not deleted - pending a launch decision (the entire "For Women" lineup, oral minoxidil, women's oral compound, women's topical spray, is commented out too, for the same reason). `/hair-loss` itself does not carry commented-out pricing cards the way the old `/hairloss` page did - it uses the simpler hub `LINEUP` array shape (see below); add a real entry there once a second hair-loss product ships. The commented-out SKUs' plans (oral minoxidil and both topical sprays had Monthly + Quarterly; the women's oral compound was Monthly only) are unchanged in `simple-treatment-pricing.ts`, just unused while disabled.
...
**Nav:** see "Nav: category dropdowns" below for the current (2026-09-03) shape - sectioned dropdowns for Hair Loss and Sexual Health, flat for Weight Loss, no dropdown for the paused Wellness category. Nav links go straight to money pages (see "Money-page architecture" below), never through `/hair-loss` or `/ed` first.
...
`/tirzepatide`, `/semaglutide`, `/tadalafil`, `/sildenafil`, `/oral-finasteride`, `/ed-mints` are money pages. Nav dropdowns (`SEXUAL_HEALTH_SECTIONS`, `HAIR_SECTIONS` in `SiteHeader.tsx`) link straight to these - never through `/ed` or `/hair-loss` first. Each money page's `resolveCta()` call targets that product's own `CtaId` (`tadalafil_hero`/`_footer`, `sildenafil_hero`/`_footer`, `oral_finasteride_hero`/`_footer`, `ed_mints_rdt_*`, `ed_mints_odt_*`), which resolves to that product's own Bask URL.
...
- `ed_hero`/`ed_footer` stay defined in `CTA_IDS`/`CTA_OVERRIDES` (unused by any live page now) rather than removed - same "kept defined while dormant" convention as the paused TRT/NAD+/sermorelin CTAs, in case a page-level CTA comes back.

`/hair-loss` works differently: it's a **category hub** (see "Category hub pages" below), using `resolveCta()` directly (`hair_hero`/`hair_footer`, resolving straight to `FINASTERIDE_INTAKE_URL`) rather than an on-page jump, since it only has one live product to route to. `hairloss_hero`/`hairloss_footer` (the old standalone `/hairloss` overview page's CTAs, already unused before the merge) were deleted, not kept dormant, when `/hairloss` was retired 2026-09-04 - unlike `/ed`'s CTAs, they had never resolved on any live page in production, so there was nothing to preserve.
...
| Hair Loss | `/hair-loss` | Live | `/oral-finasteride` (money page directly, 2026-09-03; designed to grow, see below) |
| Wellness | `/wellness` | **Paused** (redirects home) | Would link `/nad-plus`, `/sermorelin` - both paused |

**Why Hair Loss gets a full hub despite having one product page today:** more hair-loss formulations are planned, so `/hair-loss` and its nav dropdown (`HAIR_SECTIONS` in `SiteHeader.tsx`) exist now to grow into, rather than retrofitting a hub later once a second hair product ships. Add new hair-loss medication pages to `HAIR_SECTIONS` and the `/hair-loss` route's `LINEUP` array (not the footer Care column - see "Money-page architecture" above for why individual product pages stay out of the footer). `CategoryLineupItem` (the `LINEUP` array's item type, in `TreatmentPageBlocks.tsx`) supports an optional `perDayNote` field - pass `simplePerDaySentence(pricing)` on any entry whose monthly rate clears the "less than $1 a day" bar (2026-09-04, matching the tag already shown on `/oral-finasteride` itself) to show the same highlighted tag next to that entry's price.
...
| Hair Loss | `HAIR_SECTIONS` | sectioned (2026-08-28) | **For Men**: Oral Finasteride only (2026-09-03, → `/oral-finasteride`, not `/hair-loss`) - Oral Minoxidil and Topical Spray are commented out pending a launch decision. **For Women**: entire section commented out (no women's hair loss product launching yet) |
...
- `/tirzepatide`, `/semaglutide`, `/oral-finasteride`, `/tadalafil`, `/sildenafil`, `/ed-mints`, `/ed` — `BreadcrumbList` + `FAQPage` + `serviceJsonLd()` (each has a visible FAQ accordion that matches its JSON-LD - true for the overview page `/ed` too, not just the money pages). `/trt`, `/nad-plus`, `/sermorelin` carried the same shape before being paused - their JSON-LD is dormant along with the rest of the page (redirect stubs render nothing). `/hairloss` carried this shape too before being merged into `/hair-loss` (below) on 2026-09-04
...
| `src/lib/simple-treatment-pricing.ts` | Single source of truth for the non-GLP-1 products' pricing (Monthly/Quarterly, no promo code). TRT/NAD+/sermorelin consts stay defined but unused while paused; `ED_COMBO_PRICING` stays defined but superseded by `ED_MINTS_RDT_PRICING` |
| `src/routes/oral-finasteride.tsx`, `tadalafil.tsx`, `sildenafil.tsx`, `ed-mints.tsx` | The 4 live non-GLP-1 **money pages** (2026-09-03) |
...
| `src/components/site/SiteHeader.tsx`, `SiteFooter.tsx` | Weight Loss / Sexual Health / Hair Loss / More dropdowns (`DesktopNavDropdown` / `MobileNavDropdown`, `sections` prop for For Men/Women); footer Care + Resources + Trust columns (Trust includes the Google review ask) |
